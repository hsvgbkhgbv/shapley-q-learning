...
class ShapleyQMixer(nn.Module):
    def __init__(self, args):
        ...
        self.sample_grandcoalitions()

    def sample_grandcoalitions(self):
        """
        E.g. sample_size = 2, n_agents = 3:
        ...
        grand_coalitions = th.multinomial(th.ones(self.sample_size, 
                                          self.n_agents).cuda()/self.n_agents, 
                                          self.n_agents, 
                                          replacement=False)
        individual_map = th.zeros(self.sample_size*self.n_agents, self.n_agents).cuda()
        individual_map.scatter_(1, grand_coalitions.contiguous().view(-1, 1), 1)
        individual_map = individual_map.contiguous().view(self.sample_size, self.n_agents, self.n_agents)
        subcoalition_map = th.matmul(individual_map, seq_set)
        grand_coalitions = grand_coalitions.unsqueeze(1).expand(self.sample_size, 
                                                                self.n_agents, 
                                                                self.n_agents).contiguous().view(self.sample_size, 
                                                                                                 self.n_agents, 
                                                                                                 self.n_agents) # shape = (n_s, n, n)
        self.subcoalition_map = subcoalition_map
        self.individual_map = individual_map
        self.grand_coalitions = grand_coalitions

    def get_sampled_grandcoalitions(self, batch_size):
        subcoalition_map = self.subcoalition_map.unsqueeze(0).expand(batch_size,
                                                                     self.sample_size, 
                                                                     self.n_agents, 
        ...
        individual_map = self.individual_map.unsqueeze(0).expand(batch_size,
                                                                     self.sample_size, 
                                                                     self.n_agents, 
                                                                     self.n_agents).contiguous().view(batch_size,
                                                                                                      self.sample_size, 
                                                                                                      self.n_agents, 
                                                                                                      self.n_agents) # shape = (b, n_s, n, n)
        grand_coalitions = self.grand_coalitions.unsqueeze(0).expand(batch_size,
                                                                     self.sample_size, 
                                                                     self.n_agents, 
                                                                     self.n_agents).contiguous().view(batch_size,
                                                                                                      self.sample_size, 
    ...
    def get_w_estimate(self, states, agent_qs):
        ...
        subcoalition_map, individual_map, grand_coalitions = self.get_sampled_grandcoalitions(batch_size) # shape = (b, n_s, n, n)
        ...
        agent_qs_individual = agent_qs # shape = (b, n, 1)

        reshape_agent_qs_coalition_norm_vec = agent_qs_coalition_norm_vec.contiguous().view(-1, 1) # shape = (b*n, 1)
        reshape_agent_qs_individual = agent_qs_individual.contiguous().view(-1, 1) # shape = (b*n, 1)
        ...
        inputs = th.cat([reshape_agent_qs_coalition_norm_vec, reshape_agent_qs_individual], dim=-1).unsqueeze(1) # shape = (b*n, 1, 2*1)
