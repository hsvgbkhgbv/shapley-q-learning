...
class ShapleyQMixer(nn.Module):
    def __init__(self, args):
        ...
        if getattr(args, "hypernet_layers", 1) == 1:
            self.hyper_w_1 = nn.Linear(self.state_dim, self.embed_dim * (self.n_agents+1))
            self.hyper_w_final = nn.Linear(self.state_dim, self.embed_dim)
        elif getattr(args, "hypernet_layers", 1) == 2:
            hypernet_embed = self.args.hypernet_embed
            self.hyper_w_1 = nn.Sequential(nn.Linear(self.state_dim, hypernet_embed),
                                           nn.ReLU(),
                                           nn.Linear(hypernet_embed, self.embed_dim * (self.n_agents+1)))
    ...
    def get_w_estimate(self, states, agent_qs):
        ...
        agent_qs_coalition_ = agent_qs_coalition.contiguous().squeeze(-1) # shape = (b, n_s, n, n)
        ...
        agent_qs_individual = agent_qs.unsqueeze(1).expand(batch_size, self.sample_size, self.n_agents, 1) # shape = (b, n_s, n, 1)

        reshape_agent_qs_coalition_norm_vec = agent_qs_coalition_.contiguous().view(-1, self.n_agents) # shape = (b*n_s*n, n)
        ...
        w1 = w1.view(-1, self.n_agents+1, self.embed_dim)
