...
class ShapleyQMixer(nn.Module):
    def __init__(self, args):
        ...
        if getattr(args, "hypernet_layers", 1) == 1:
            self.hyper_w_1 = nn.Linear(self.state_dim+2*self.n_actions, self.embed_dim * 2)
            self.hyper_w_final = nn.Linear(self.state_dim+2*self.n_actions, self.embed_dim)
        elif getattr(args, "hypernet_layers", 1) == 2:
            hypernet_embed = self.args.hypernet_embed
            self.hyper_w_1 = nn.Sequential(nn.Linear(self.state_dim+2*self.n_actions, hypernet_embed),
                                           nn.ReLU(),
                                           nn.Linear(hypernet_embed, self.embed_dim * 2))
            self.hyper_w_final = nn.Sequential(nn.Linear(self.state_dim+2*self.n_actions, hypernet_embed),
                                           nn.ReLU(),
        ...
        self.hyper_b_1 = nn.Linear(self.state_dim+2*self.n_actions, self.embed_dim)
        # V(s) instead of a bias for the last layers
        self.V = nn.Sequential(nn.Linear(self.state_dim+2*self.n_actions, self.embed_dim),
                            nn.ReLU(),
    ...
    def get_w_estimate(self, states, agent_qs, agent_actions):
        ...
        grand_coalitions_qs = grand_coalitions.unsqueeze(-1).expand(batch_size, 
                                                                 self.sample_size, 
                                                                 self.n_agents, 
                                                                 self.n_agents, 
                                                                 1) # shape = (b, n_s, n, n, 1)

        grand_coalitions_actions = grand_coalitions.unsqueeze(-1).expand(batch_size, 
                                                                 self.sample_size, 
                                                                 self.n_agents, 
                                                                 self.n_agents, 
        ...
        subcoalition_map_no_i_qs = subcoalition_map_no_i.unsqueeze(-1).expand(batch_size, 
                                                                 self.sample_size, 
                                                                 self.n_agents, 
                                                                 self.n_agents, 
                                                                 1) # shape = (b, n_s, n, n, 1)
        subcoalition_map_no_i_actions = subcoalition_map_no_i.unsqueeze(-1).expand(batch_size, 
                                                                 self.sample_size, 
                                                                 self.n_agents, 
                                                                 self.n_agents, 
                                                                 self.n_actions) # shape = (b, n_s, n, n, a)
        ...
        reshape_agent_qs = agent_qs.unsqueeze(1).unsqueeze(2).expand(batch_size, 
                                                        ...
                                                        self.n_agents, 
                                                        self.n_agents, 
                                                        1).gather(3, grand_coalitions_qs) # shape = (b, n, 1) -> (b, 1, 1, n, 1) -> (b, n_s, n, n, 1)

        reshape_agent_actions = agent_actions.unsqueeze(1).unsqueeze(2).expand(batch_size, 
                                                        self.sample_size, 
                                                        self.n_agents, 
                                                        self.n_agents, 
                                                        self.n_actions).gather(3, grand_coalitions_actions) # shape = (b, n, a) -> (b, 1, 1, n, a) -> (b, n_s, n, n, a)

        # get actions of its coalition memebers for each agent
        agent_qs_coalition = reshape_agent_qs * subcoalition_map_no_i_qs # shape = (b, n_s, n, n, 1)
        agent_actions_coalition = reshape_agent_actions * subcoalition_map_no_i_actions # shape = (b, n_s, n, n, a)

        # get actions vector of its coalition members for each agent
        subcoalition_map_no_i_qs_ = subcoalition_map_no_i_qs.sum(dim=-2).clone()
        subcoalition_map_no_i_qs_[subcoalition_map_no_i_qs.sum(dim=-2)==0] = 1
        agent_qs_coalition_norm_vec = agent_qs_coalition.sum(dim=-2) / subcoalition_map_no_i_qs_ # shape = (b, n_s, n, 1)

        subcoalition_map_no_i_actions_ = subcoalition_map_no_i_actions.sum(dim=-2).clone()
        subcoalition_map_no_i_actions_[subcoalition_map_no_i_actions.sum(dim=-2)==0] = 1
        agent_actions_coalition_norm_vec = agent_actions_coalition.sum(dim=-2) / subcoalition_map_no_i_actions_ # shape = (b, n_s, n, 1)
        ...
        agent_actions_individual = agent_actions.unsqueeze(1).expand_as(agent_actions_coalition_norm_vec) # shape = (b, n_s, n, a)
        ...
        reshape_agent_actions_coalition_norm_vec = agent_actions_coalition_norm_vec.contiguous().view(-1, self.n_actions) # shape = (b*n_s*n, a)
        reshape_agent_actions_individual = agent_actions_individual.contiguous().view(-1, self.n_actions) # shape = (b*n_s*n, a)
        ...
        inputs_ = th.cat([reshape_states, reshape_agent_actions_coalition_norm_vec, reshape_agent_actions_individual], dim=-1)

        # First layer
        w1 = th.abs(self.hyper_w_1(inputs_))
        b1 = self.hyper_b_1(inputs_)
        w1 = w1.view(-1, 2, self.embed_dim)
        b1 = b1.view(-1, 1, self.embed_dim)
        hidden = F.elu(th.bmm(inputs, w1) + b1)
        # Second layer
        w_final = th.abs(self.hyper_w_final(inputs_))
        w_final = w_final.view(-1, self.embed_dim, 1)
        # State-dependent bias
        v = self.V(inputs_).view(-1, 1, 1)
