...
class ShapleyQMixer(nn.Module):
    def __init__(self, args):
        ...
        self.prob_w = nn.Linear(self.state_dim+2*self.n_actions, 1)
    ...
    def get_w_estimate(self, states, agent_qs, agent_actions):
        ...
        # get prob for normalise among samples
        inputs_ = th.cat([reshape_states, reshape_agent_actions_coalition_norm_vec, reshape_agent_actions_individual], dim=-1) # shape = (b*n_s*n, s+2*a)
        prob = self.prob_w(inputs_) # shape = (b*n_s*n, 1)
        prob = prob.squeeze(-1).contiguous().view(batch_size, self.sample_size, self.n_agents) # shape = (b, n_s, n)
        prob = th.softmax(prob, dim=1)
