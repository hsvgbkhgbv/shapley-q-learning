...
class ShapleyQMixer(nn.Module):
    ...
    def get_coalition_actions_enc(self, actions):
        ...
        coalition_actions_enc = coalition_actions_enc.contiguous().view(-1, self.n_agents, self.embed_dim)
    ...
    def get_individual_actions_enc(self, actions):
        ...
        individual_actions_enc = individual_actions_enc.contiguous().view(-1, self.n_agents, self.embed_dim)
    ...
    def get_w_estimate(self, states, actions):
        ...
        state_embed = self.get_states_enc(states) # shape = (b, e)
        actions_embed = self.get_individual_actions_enc(actions.contiguous().view(-1, self.n_actions)).contiguous().view(batch_size, self.n_agents, self.embed_dim) # shape = (b, n, e)

        # reshape actions for further process on coalitions
        reshape_actions_embed = actions_embed.unsqueeze(1).unsqueeze(2).expand(batch_size, 
                                                                self.sample_size, 
                                                                self.n_agents, 
                                                                self.n_agents, 
        ...
        actions_coalition = reshape_actions_embed * subcoalition_map_no_i # shape = (b, n_s, n, n, e)

        # get actions vector of its coalition members for each agent
        actions_coalition_norm_vec = actions_coalition.mean(dim=-2) * subcoalition_map_no_i.sum(dim=-2) # shape = (b, n_s, n, e)
        actions_coalition_norm_vec = actions_coalition_norm_vec.mean(dim=1) # shape = (b, n, e)

        # get action vector of each agent
        actions_individual = actions_embed / self.n_agents # shape = (b, n, e)

        reshape_actions_coalition_norm_vec = actions_coalition_norm_vec.contiguous().view(-1, self.embed_dim) # shape = (b*n, e)
        reshape_actions_individual = actions_individual.contiguous().view(-1, self.embed_dim) # shape = (b*n, e)
        reshape_states = state_embed.unsqueeze(1).expand(batch_size, self.n_agents, self.embed_dim).contiguous().view(-1, self.embed_dim) # shape = (b*n, e)
        ...
        inputs = th.cat([reshape_states, reshape_actions_coalition_norm_vec, reshape_actions_individual], dim=-1) # shape = (b*n, 3*e)
