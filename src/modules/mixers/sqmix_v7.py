...
class ShapleyQMixer(nn.Module):
    ...
    def get_w_estimate(self, states, agent_qs):
        ...
        agent_qs_coalition = reshape_agent_qs * subcoalition_map_no_i # shape = (b, n_s, n, n, a)
        ...
        agent_qs_coalition_norm_vec = agent_qs_coalition.sum(dim=-2) / subcoalition_map_no_i_ # shape = (b, n_s, n, a)
        ...
        agent_qs_individual = agent_qs.unsqueeze(1).expand_as(agent_qs_coalition_norm_vec) # shape = (b, n_s, n, a)

        reshape_agent_qs_coalition_norm_vec = agent_qs_coalition_norm_vec.contiguous().view(-1, self.n_actions) # shape = (b*n_s*n, a)
        reshape_agent_qs_individual = agent_qs_individual.contiguous().view(-1, self.n_actions) # shape = (b*n_s*n, a)
        ...
        inputs = th.cat([reshape_agent_qs_coalition_norm_vec, reshape_agent_qs_individual], dim=-1).unsqueeze(1) # shape = (b*n_s*n, 1, 2*a)
    ...
    def forward(self, states, actions, agent_qs, max_filter, target=True):
        ...
        reshape_agent_qs = actions.contiguous().view(-1, self.n_agents, self.n_actions).float()
        if target:
